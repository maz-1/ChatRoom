...
import { parseTunnelControlMessage } from "../../src/plugins/cloud/tunnel-client.js";
...
test("CloudController serializes concurrent status mutations", async () => {
  ...
  try {
    ...
    const controller = await CloudController.create(config);
    let active = 0;
    let maximumActive = 0;
    let calls = 0;
...
test("Cloud state failure degrades without aborting application startup", async () => {
  ...
  try {
    components = await createApplication(config);
