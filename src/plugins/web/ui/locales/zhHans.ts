import type { enChatRoom } from "./en.js";

export const zhChatRoom: typeof enChatRoom = {
  nav: {
    operations: "操作日志",
    systemLogs: "系统日志",
    workspaces: "工作区",
    processes: "进程",
    computer: "电脑控制",
    mcp: "MCP 桥接",
    mcpTools: "MCP 工具",
    oauthClients: "OAuth 客户端",
    cloud: "Cloud",
  },
  mcpTools: {
    title: "MCP 工具访问",
    refresh: "刷新工具",
    togglePlugin: "切换全部 {0} 工具",
    empty: "当前没有已注册的 MCP 工具。",
    plugins: {
      workspace: "工作区",
      process: "进程",
      computer: "电脑控制",
    },
  },
  oauthClients: {
    title: "OAuth 客户端",
    subtitle: "已注册的 MCP OAuth 客户端、运行时访问状态与客户端控制",
    summary: "{0} / {1} 活跃",
    refresh: "刷新客户端",
    client: "客户端",
    note: "备注",
    noNote: "暂无备注",
    editNote: "编辑备注",
    noteTitle: "编辑客户端备注",
    noteHint: "最多 1000 个字符",
    saveNote: "保存备注",
    redirectUris: "回调地址",
    showAllRedirects: "查看全部（{0}）",
    collapseRedirects: "收起",
    createdAt: "创建时间",
    lastAccessAt: "最近访问",
    runtimeOnly: "仅本次运行",
    status: "状态",
    disabled: "禁用",
    actions: "操作",
    toggleDisabled: "启用或禁用 {0}",
    revoke: "吊销",
    revokeTitle: "吊销 OAuth 客户端",
    revokeConfirm:
      "确定吊销并删除 {0} 吗？客户端记录、access token、refresh token 和 authorization code 会立即删除，且无法恢复。",
    empty: "当前没有已注册的 OAuth 客户端。",
    statuses: {
      active: "正常",
      disabled: "已禁用",
    },
  },
  mcp: {
    title: "MCP 桥接",
    subtitle: "经 ChatRoom 代理的服务及其对外暴露的工具",
    server: "服务",
    transport: "传输方式",
    enabled: "启用",
    status: "状态",
    tools: "工具",
    toggleServer: "启用或禁用 {0}",
    reconnect: "重新连接",
    reconnectAll: "全部重连",
    summary: "{0} / {1} 已连接",
    select: "选择左侧的一个服务以查看它的工具。",
    empty: "尚未配置任何 MCP 桥接服务。",
    noTools: "该服务没有暴露任何工具。",
    stderr: "服务 stderr 输出",
    addTitle: "添加服务",
    addSubtitle: "服务定义位于 ChatRoom 配置文件中",
    addBody:
      "在下方配置文件的 mcp.servers 中新增一项。通过 env 或 headers 传入的凭据只留在本机，不会显示在此页面。",
    addNotice: "配置仅在启动时读取：修改后需重启 ChatRoom。",
    statuses: {
      connected: "已连接",
      disabled: "已禁用",
      error: "不可用",
    },
  },
  computer: {
    title: "电脑控制",
    platform: "平台",
    helper: "原生辅助程序",
    access: "访问控制",
    enabled: "启用电脑控制",
    remoteAccess: "允许远程访问",
    testSnapshot: "测试截图",
    displays: "显示器",
    primaryDisplay: "主显示器",
    latestScreen: "画面",
    capturedAt: "获取时间：{0}",
    refreshScreen: "刷新画面",
    noSnapshot: "暂无截图。启用电脑控制后刷新画面。",
    remoteDisabled: "远程电脑控制未开启。开启远程访问后即可查看和控制此电脑。",
    activeApp: "当前应用",
    activeWindow: "当前窗口",
    elements: "可访问元素",
    recentOperations: "最近电脑控制操作",
    refreshOperations: "刷新最近操作",
    noOperations: "暂无电脑控制操作。",
    accessibility: "辅助功能",
    screenRecording: "屏幕与系统音频录制",
    requestPermission: "申请",
    platformStates: {
      macos: "macOS",
      windows: "Windows",
      linux: "Linux X11",
      unsupported: "不支持",
    },
    helperStates: {
      running: "运行中",
      stopped: "已停止",
      unavailable: "不可用",
    },
    permissionStates: {
      granted: "已授权",
      denied: "未授权",
      unknown: "未知",
      "not-required": "无需授权",
    },
    permissionLocalOnly:
      "系统权限只能在运行 ChatRoom 的这台电脑上通过本地 WebUI 申请，公网访问无法发起授权请求。",
    log: {
      snapshot: "查看了当前屏幕",
      settings: "修改了电脑控制设置",
      action: "操作了电脑",
      activateApp: "激活应用 {0}",
      activateWindow: "激活窗口 {0}",
      click: "点击 {0}",
      doubleClick: "双击 {0}",
      rightClick: "右键点击 {0}",
      move: "移动指针到 {0}",
      drag: "拖动指针",
      scroll: "滚动 {0}",
      keypress: "按下 {0}",
      typeText: "输入 {0} 个字符",
      invoke: "执行 {0}",
      selectText: "在 {0} 中选择文本",
      moveWindow: "移动窗口 {0}",
      resizeWindow: "调整窗口 {0}",
      wait: "等待 {0} 毫秒",
    },
  },
  cloud: {
    inactive: "尚未启用",
    inactiveDescription: "购买或恢复 ChatRoom Cloud 后即可使用。",
    purchase: "购买",
    manage: "管理",
    refresh: "刷新",
    restore: "恢复订阅",
    recoveryKey: "恢复密钥",
    restoreAction: "恢复",
    subscription: "订阅状态",
    active: "已生效",
    expires: "到期时间",
    installationId: "安装 ID",
    remoteMcp: "远程 MCP",
    remoteMcpDescription: "通过 ChatRoom Cloud 暴露 MCP。",
    remoteWeb: "远程 WebUI",
    remoteWebDescription: "通过 ChatRoom Cloud 暴露 ChatRoom WebUI。",
    disableMcpTitle: "关闭远程 MCP？",
    disableMcpDescription:
      "关闭后将停止远程 MCP 访问，重新开启后才能继续使用。",
    disableWebTitle: "关闭远程 WebUI？",
    disableWebDescription:
      "关闭后，当前远程 WebUI 地址将立即无法访问，也无法再通过该地址返回此页面重新开启。请通过本地 WebUI 重新开启远程访问。",
    disableConfirm: "关闭",
    connection: {
      inactive: "未启用",
      connecting: "连接中",
      connected: "已连接",
      disconnected: "已断开",
      error: "错误",
    },
  },
  runtime: {
    version: "运行时版本",
    updateAvailable: "可更新",
    updateTitle: "新版本 {0} 可用",
    mcpRequests: "MCP 请求",
    uptime: "运行时间",
    uptimeHoursMinutes: "{0} 小时 {1} 分钟",
  },
  common: {
    connection: {
      connecting: "连接中",
      connected: "已连接",
      reconnecting: "正在重连",
      offline: "离线",
    },
    signOut: "退出登录",
    language: "语言",
    theme: "主题",
    cancel: "取消",
    close: "关闭",
  },
  theme: { system: "跟随系统", light: "浅色", dark: "深色" },
  auth: {
    ownerAccess: "所有者访问",
    ownerToken: "所有者令牌",
    signIn: "登录",
    signInWithPasskey: "使用通行密钥登录",
    orToken: "或使用所有者令牌",
    rememberDevice: "在此设备保持登录 30 天",
    passkeys: "通行密钥",
    passkeyDescription:
      "使用 Face ID、Touch ID、Windows Hello 或安全密钥登录。",
    passkeyName: "通行密钥名称",
    thisDevice: "此设备",
    addPasskey: "添加通行密钥",
    removePasskey: "删除通行密钥",
    noPasskeys: "尚未注册通行密钥。",
    lastUsed: "最近使用",
  },
  operations: {
    title: "插件操作日志",
    subtitle: "ChatRoom 各插件产生的操作记录",
    all: "全部",
    running: "运行中",
    errors: "错误",
    success: "成功",
    empty: "暂无操作记录。",
    select: "选择一条操作查看详情。",
    clear: "清空记录",
    clearTitle: "清空操作记录？",
    clearDescription:
      "已完成的操作记录将被永久删除。正在运行的操作会保留，直到操作结束。",
    clearConfirm: "清空记录",
    back: "返回操作日志",
  },
  systemLogs: {
    title: "系统日志",
    subtitle: "ChatRoom 运行时、认证、Cloud 与插件事件",
    allLevels: "全部级别",
    allModules: "全部模块",
    refresh: "刷新日志",
    empty: "暂无系统日志。",
    select: "选择一条日志查看详情。",
    loadMore: "加载更早日志",
    back: "返回系统日志",
    time: "时间",
    level: "级别",
    module: "模块",
    event: "事件",
    message: "消息",
    data: "详细信息",
  },
  table: {
    time: "时间",
    action: "操作",
    plugin: "插件",
    source: "来源",
    status: "状态",
    duration: "耗时",
  },
  detail: {
    plugin: "插件",
    duration: "耗时",
    process: "进程",
    input: "输入",
    output: "输出",
    error: "错误",
  },
  workspaces: {
    prompt: "提示词",
    files: "文件",
    skills: "技能",
    empty: "配置的 allowedRoots 下没有发现工作区。",
    createTitle: "新建工作区",
    refresh: "刷新工作区",
    projectName: "项目名称",
    createLocation: "创建位置",
    projectPath: "项目路径",
    create: "创建",
    cancel: "取消",
    summary: "项目概况",
    presetPrompt: "预设提示词",
    save: "保存",
    saved: "工作区元数据已保存。",
  },
  git: {
    notRepository: "这不是 Git 仓库",
    branch: "分支",
    upstream: "上游",
    sync: "同步状态",
    branches: "分支",
    refresh: "刷新仓库",
    fetch: "获取",
    pull: "拉取",
    push: "推送",
    changes: "修改",
    stage: "暂存",
    unstage: "取消暂存",
    stageAll: "全部暂存",
    unstageAll: "全部取消暂存",
    restore: "回退",
    deleteFile: "删除",
    clean: "工作区干净",
    commitMessage: "提交信息",
    commit: "提交",
    recentCommits: "最近提交",
    newBranch: "新建分支",
    create: "创建",
    current: "当前",
    switch: "切换",
    diffTruncated: "Diff 预览已截断。",
    noDiff: "没有可显示的 Diff。",
    noHead: "该仓库还没有任何提交。",
    restoreTitle: "回退这个文件的修改？",
    restoreDescription:
      "该操作会将文件恢复到 HEAD，并永久丢弃已暂存和未暂存的修改。",
    deleteTitle: "删除未跟踪文件？",
    deleteDescription: "该文件未被 Git 跟踪，删除后无法通过 Git 恢复。",
    confirm: "确认",
    deleteBranch: "删除分支",
    deleteBranchTitle: "删除分支？",
    deleteBranchDescription: "只有当该分支的提交已经合并时，Git 才会执行删除。",
  },
  files: {
    select: "选择文件",
    readOnly: "文件浏览为只读。",
    file: "文件",
    directory: "目录",
    symlink: "符号链接",
    previewUnavailable: "暂不支持预览",
    parentDirectory: "上级目录",
    refresh: "刷新目录",
  },
  skills: {
    empty: "未发现 Skill。",
    noDescription: "SKILL.md 元数据中未提供描述。",
  },
  processes: {
    title: "进程",
    subtitle: "受监管命令及其当前状态",
    command: "命令",
    arguments: "参数",
    fullCommand: "完整命令",
    state: "状态",
    started: "启动时间",
    duration: "耗时",
    terminate: "终止",
    kill: "强制结束",
    moreActions: "更多操作",
    empty: "暂无受监管进程。",
    exit: "退出码",
    timeout: "超时",
    yes: "是",
    no: "否",
    select: "选择一个进程查看输出。",
    back: "返回进程列表",
  },
  code: {
    search: "搜索",
    noOutput: "暂无输出。",
    copy: "复制",
    enableWrap: "启用自动换行",
    disableWrap: "关闭自动换行",
    download: "下载",
  },
  sources: { mcp: "模型", gui: "网页界面", cli: "命令行", system: "运行时" },
  statuses: {
    running: "运行中",
    success: "成功",
    exited: "已退出",
    error: "错误",
    failed: "失败",
    cancelled: "已取消",
    killed: "已结束",
  },
  actions: {
    workspaceInfo: "读取工作区信息",
    gitStage: "暂存 Git 修改",
    gitUnstage: "取消暂存 Git 修改",
    gitRestore: "回退 Git 文件",
    gitCommit: "提交 Git 修改",
    gitBranchCreate: "创建 Git 分支",
    gitBranchSwitch: "切换 Git 分支",
    gitBranchDelete: "删除 Git 分支",
    gitFetch: "获取 Git 远程更新",
    gitPull: "拉取 Git 修改",
    gitPush: "推送 Git 修改",
    startProcess: "启动进程",
    readProcess: "读取进程输出",
    writeProcess: "写入进程输入",
    stopProcess: "停止进程",
    terminateProcess: "终止进程",
    forceStopProcess: "强制结束进程",
    syncCloud: "同步 Cloud 状态",
    manageCloud: "打开 Cloud 管理页面",
    restoreCloud: "恢复 Cloud 订阅",
    replaceRecoveryKey: "更换恢复密钥",
    setCloudService: "设置 Cloud 服务",
  },
};
